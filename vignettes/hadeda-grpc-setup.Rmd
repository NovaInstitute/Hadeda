...
"protobuf"), stdout = TRUE), "include")`. Extend `proto_paths` with that
directory and reload the services:

```{r}
google_proto_path <- file.path(system2("brew", c("--prefix", "protobuf"), stdout = TRUE), "include")
proto_paths <- unique(c(proto_paths, google_proto_path))
...
`google/protobuf` from your Protocol Buffers installation before rerunning the
service loader.

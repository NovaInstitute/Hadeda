...
```{r eval=FALSE}
HOK <- Sys.getenv("HADEDA_OPERATOR_KEY")
fp <- hadeda_write_operator_key(HOK)
```
...
sign_transaction <- hadeda_grpc_ed25519_signer(fp)
...
```{r}
info <- crypto_get_account_info(config = grpc_config, account_id = operator$operator_account_id)
t6info
```

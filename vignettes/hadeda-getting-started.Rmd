...
transaction signing handler in `config$grpc`. The package does not prescribe a
single signing strategy; instead, it works with closures that return signed
transactions.
...
With transports configured, you can create an account using the Hashio REST
fallback or the CryptoService gRPC helper. REST providers such as Hashio manage
key generation for you:

```{r}
new_account <- accounts_create(
  mirror,
  initial_balance = 10,
  memo = "Hadeda demo"
)
...
When you control signing keys you can submit the same request over gRPC. The
helper returns acknowledgement metadata including the new account ID and the
full gRPC response payload for inspection.

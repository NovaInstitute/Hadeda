#' List approved fungible token allowances for an account
#'
#' Retrieve token allowances granted by an account via the Hedera Mirror Node REST API.
#'
#' @param config A configuration list created by [hadeda_config()].
#' @param account_id The account whose token allowances should be retrieved.
...
#'
#' @return A tibble of token allowances granted by the account.
#'
#' @examples
#' mirror <- hadeda_config(network = "testnet")
#' \dontrun{
#'   accounts_allowances_tokens(
#'     mirror,
#'     account_id = "0.0.1234",
#'     token_id = "0.0.5678",
#'     spender_id = "0.0.9876"
...
#'
#' @export
accounts_allowances_tokens <- function(config,
                                        account_id,
                                        limit = NULL,
                                        spender_id = NULL,
                                        token_id = NULL,
  ...
  if (!identical(transport, "rest")) {
    cli::cli_abort("accounts_allowances_tokens() currently supports only the REST transport.")
  }

  if (missing(account_id) || is.null(account_id)) {
    cli::cli_abort("`account_id` must be supplied to accounts_allowances_tokens().")
  }
  ...
  path <- paste0("accounts/", account_id, "/allowances/tokens")

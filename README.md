...
## Debug logging with `hadeda_debug`

Hadeda ships a small set of internal helpers that surface verbose diagnostics
when you opt in to debugging. Any package code can emit structured messages via
`hadeda_debug()` or `hadeda_debug_list()`, but the output stays silent unless one
of the following toggles is enabled:

- Set the R option `hadeda.debug = TRUE` before calling into the package. This
  is convenient for ad-hoc sessions, for example:
...
- Export an environment variable before launching R:

  ```bash
  export HADEDA_DEBUG=1  # also accepts "true", "yes", or "on"
  R
  ```

Once enabled, any debug messages are routed through `{cli}` with the `debug`
class, so they are visually distinct from `cli_inform()` output generated by
normal operations. Structured objects passed to `hadeda_debug_list()` are
rendered with `str()` to make it easy to inspect request payloads and parsed
responses while troubleshooting gRPC interactions.
